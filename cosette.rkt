...
;; Return a new list with the first least element incremented
(define (inc-table-sizes sizes)
  (define (inc-helper m sizes)
    (match sizes
      ['() (list '() #f)]
      [(cons s sizes)
       (match-let ([(list new-sizes mod?) (inc-helper (min m s) sizes)]
                   [lower? (< s m)])
         (list (cons (+ s (if (and lower? (not mod?)) 1 0)) new-sizes)
               (or lower? mod?)))]))
  (car (inc-helper +inf.0 sizes)))
...
(define (solve-relations r1 r2 table-infos messenger)
  (let* ([initial-sizes (map (lambda (_) 1) table-infos)]
         [try-solve
           (lambda (table-sizes)
             (let* ([tables (init-sym-tables table-infos table-sizes)]
                    [solution (verify (bag-equal (r1 tables) (r2 tables)))])
               (if (sat? solution)
                   (cons "NEQ" (evaluate tables solution))
                   (cons "EQ" '()))))])
    (define (rec-wrapper table-sizes)
      (let ([sol (try-solve table-sizes)])
        (cond [(eq? (car sol) "NEQ") (messenger sol)]
              [else (messenger table-sizes)
                    (rec-wrapper (inc-table-sizes table-sizes))])))
    (rec-wrapper initial-sizes)))
